...
public class NetworkServer implements Closeable {
    ...
    private NetworkServer(HostAndPort listenInterfaceAndPort, MessageHandler messageHandler) {
     ...
     */
    public static NetworkServer create(HostAndPort listenInterfaceAndPort, MessageHandler messageConsumer) {
        return new NetworkServer(listenInterfaceAndPort, messageConsumer);
    }

    public HostAndPort getListenAddress() {
        return serverConnector.getListenInterfaceAndPort();
    }
    ...
    public void start() throws IOException, ServerException {
        ...
        Consumer<Message> consumer = message -> {
            LoggerFactory.getLogger(NetworkServer.class).debug("Receive reply message {}", message.getId());

            boolean send = send(message);

            LoggerFactory.getLogger(NetworkServer.class).debug("Message with id: {} was send with status: {}", message.getId(), send);
        };
    ...
    protected boolean send(Message message) {
        try {
            ...
            LoggerFactory.getLogger(NetworkServer.class).warn("Can't send data to client", e);
